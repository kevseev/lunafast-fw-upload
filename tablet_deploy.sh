...
load_settings() {
	...
	'' | *[!0-9]*) LIGHT_LEVEL=5 ;;
	*)
		if [ "$LIGHT_LEVEL" -lt 1 ] || [ "$LIGHT_LEVEL" -gt 10 ]; then
			LIGHT_LEVEL=5
		fi
		;;
	esac
...
usage() {
  ...
  КриптоПро: Прошивка › HTTP API — PUT http://<IP>:HEALTH_PORT/cryptopro/upload/container (curl -F container=@файл из certs/ -F password=…), список устройств как для APK (п.3).
...
# Подсветка на устройстве: light_level 1..10
lunafast_is_light_level_1_10() {
	lunafast_is_nonneg_int "$1" || return 1
	[ "$1" -ge 1 ] && [ "$1" -le 10 ]
}
...
lunafast_light_apply_one_to_log() {
	_serial=$1
	_level=$2
	_log=$3
	if ! _host=$(lunafast_serial_http_host "$_serial"); then
		{
			printf '%s\n' "########################################"
			printf '%s\n' "# serial: $_serial — подсветка /light"
			printf '%s\n' "########################################"
			printf '%s\n' "НЕ ОК · нет сетевого host в serial (например USB)"
			printf '%s\n' ""
		} >>"$_log"
	...
	_url=$(printf 'http://%s:%s/light' "$_host" "${HEALTH_PORT:-8080}")
	_body=$(mktemp) || return 1
	_cerr=$(mktemp) || {
		rm -f "$_body"
		return 1
	}
	_payload=$(printf '{"command":"on","light_level":%s}' "$_level")
	{
		printf '%s\n' "########################################"
		printf '%s\n' "# serial: $_serial — POST /light"
		printf '%s\n' "# URL: $_url"
		printf '%s\n' "# payload: $_payload"
		printf '%s\n' "########################################"
	} >>"$_log"
	_http=$(curl --location --silent --show-error --connect-timeout 5 --max-time 30 \
		...
		--output "$_body" 2>"$_cerr") || true
	_ce=$?
	_light_resp=$(cat "$_body" 2>/dev/null) || _light_resp=
	printf '%s\n' "--- stderr curl ---" >>"$_log"
	cat "$_cerr" >>"$_log" 2>/dev/null || true
	printf '%s\n' "--- тело ответа ---" >>"$_log"
	printf '%s\n' "$_light_resp" >>"$_log"
	...
	if [ "$_ok" -eq 1 ]; then
		printf '%s\n' "Итог: ОК" >>"$_log"
		if [ -n "$_light_resp" ]; then
			printf '%s\n' "--- успех применения подсветки: ответ устройства ---" >>"$_log"
			printf '%s\n' "$_light_resp" >>"$_log"
		else
			printf '%s\n' "--- успех применения подсветки: тело ответа пусто ---" >>"$_log"
		fi
	else
		printf '%s\n' "Итог: НЕ ОК" >>"$_log"
	fi
...
lunafast_light_apply_serials_logged() {
	_serf=$1
	_level=$2
	_log=$3
	_err=0
	while read -r raw || [ -n "$raw" ]; do
		_s=$(serial_clean "$raw")
		[ -z "$_s" ] && continue
		lunafast_light_apply_one_to_log "$_s" "$_level" "$_log" || _err=1
	done <"$_serf"
	return "$_err"
}
...
dialog_http_actions_menu() {
	while true; do
		c=$(dialog --stdout --clear \
			--title "[ Прошивка › HTTP API ]" \
			--menu "Операции HTTP по выбранным устройствам (список из «Выбор устройств»)." 19 84 7 \
			1 "КриптоПро: PUT …/cryptopro/upload/container" \
			2 "Заставка: POST …/screensaver" \
			3 "Кликабельная зона: POST /click-area" \
			4 "Подсветка: POST /light (command=on, light_level=X)" \
			0 "◀ Назад в Прошивка") || break
		case "$c" in
		1) dialog_cryptopro_upload_saved ;;
		2) dialog_screensaver_upload_saved ;;
		3) dialog_click_area_saved ;;
		4) dialog_light_saved ;;
		0) break ;;
		esac
	done
}
...
dialog_light_saved() {
	...
	load_settings
	mode=$(dialog --stdout --title "[ Прошивка › подсветка /light ]" \
		--menu "Выберите режим:" 12 68 3 \
		1 "Уровень из настроек: LIGHT_LEVEL=$LIGHT_LEVEL" \
		2 "Ввести light_level вручную (1–10)" \
		0 "Отмена") || return
	case "$mode" in
	0 | "") return ;;
	1)
		...
		;;
	2)
		level=$(dialog --stdout --inputbox "light_level (целое 1–10):" 8 56 "$LIGHT_LEVEL") || return
		;;
	*) return ;;
	esac
	lunafast_is_light_level_1_10 "$level" || {
		dialog --msgbox "light_level должен быть целым от 1 до 10." 6 52
		return 1
	}
	_nd=$(wc -l <"$f" | tr -d ' ')
	dialog --yesno "Включить подсветку (command=on) на $_nd устройство(в)?\n\nlight_level=$level\nPOST …/light · порт HTTP ${HEALTH_PORT}" 11 74 || return
	rep=$(mktemp) || {
		dialog --msgbox "Не удалось создать временный файл журнала." 6 60
		return 1
	}
	if ! lunafast_begin_log_section "$rep" "Подсветка: POST /light · on · light_level=$level"; then
		rm -f "$rep"
		dialog --msgbox "Не удалось записать журнал (сессия)." 8 72
		return 1
	fi
	lunafast_light_apply_serials_logged "$f" "$level" "$rep"
	ret=$?
	lunafast_end_log_section "$rep" "$ret"
	dialog --title "[ подсветка — отчёт ]" --cr-wrap --textbox "$rep" 26 90
	lunafast_prepend_session_to_project_log "$rep" || true
	proj=$(lunafast_project_log_path)
	rm -f "$rep"
	if [ "$ret" -ne 0 ]; then
		dialog --msgbox "Есть ошибки (код $ret).\n\nСводка — в только что показанном отчёте.\nПолный журнал: $proj · главное меню п.5" 12 72
	fi
}
...
dialog_settings() {
	...
	ll=$(dialog --stdout --title "[ подсветка /light ]" \
		--inputbox "LIGHT_LEVEL по умолчанию (1–10):" 8 62 "$LIGHT_LEVEL") || return
	...
	lunafast_is_light_level_1_10 "$ll" || {
		dialog --msgbox "LIGHT_LEVEL должен быть целым от 1 до 10." 6 52
		return 1
	}
	CLICK_X1=$cx1
...
LIGHT_LEVEL — дефолт для POST /light (1–10, по умолчанию 5)." 20 74
}
...
' " lunafast ▶ Прошивка (вложенное меню)                    │"
		text_hline_mid 58
		printf '│  1) Мастер: connect → APK/XAPK → выбор                    │
'
		printf '│  2) Подключить IP:PORT                                   │
'
		printf '│  3) Выбор устройств → сохранить                           │
'
		printf '│  4) Показать сохранённые serial                          │
'
		printf '│  5) Установить APK/XAPK на сохранённый список            │
'
...
'
		text_hline 58
		printf '│ %s
' " lunafast ▶ Прошивка ▶ HTTP API                           │"
		text_hline_mid 58
		printf '│  1) КриптоПро: PUT контейнер                             │
'
		printf '│  2) Заставка: POST /screensaver                          │
'
		printf '│  3) Кликабельная зона: POST /click-area                 │
'
		printf '│  4) Подсветка: POST /light                               │
'
		printf '│  0) ◀ Назад                                              │
'
		text_hline_bot 58
		...
		read -r m || return
		case "$m" in
		1) text_cryptopro_upload_txt ;;
		2) text_screensaver_upload_txt ;;
		3) text_click_area_saved_txt ;;
		4) text_light_saved_txt ;;
		0) break ;;
		esac
	done
}
...
text_light_saved_txt() {
	...
	load_settings
	printf '%s\n' "Режим: 1) LIGHT_LEVEL из настроек ($LIGHT_LEVEL, 1–10)  2) ввести уровень  0) отмена"
	printf '%s' "? "
	read -r mode || return
	case "$mode" in
	0 | "") return ;;
	1)
		...
		;;
	2)
		printf '%s' "light_level [$LIGHT_LEVEL]: "
		read -r level || return
		[ -z "$level" ] && level=$LIGHT_LEVEL
		;;
	*)
		printf '%s\n' "Отменено."
		read -r _
	...
	lunafast_is_light_level_1_10 "$level" || {
		printf '%s\n' "light_level должен быть целым от 1 до 10."
		read -r _
		return 1
	}
	printf '%s\n' "POST /light command=on light_level=$level на устройства ниже? [y/N]"
	cat "$f"
	read -r yn || return
	case "$yn" in
	y | Y | yes | YES | д | Д) ;;
	*)
	...
	lunafast_begin_log_section "$lg" "Подсветка: POST /light · on · light_level=$level" || {
		rm -f "$lg"
		printf '%s\n' "Не удалось начать сессию журнала."
		read -r _
		return 1
	}
	lunafast_light_apply_serials_logged "$f" "$level" "$lg"
	ret=$?
	lunafast_end_log_section "$lg" "$ret"
	printf '%s\n' "=== Отчёт подсветки (сразу) · код $ret ==="
	cat "$lg"
	lunafast_prepend_session_to_project_log "$lg" || true
	proj=$(lunafast_project_log_path)
	printf '%s\n' "=== Запись добавлена в %s ===\n--- Enter ---" "$proj"
	read -r _
	rm -f "$lg"
}
...
menu_settings_text() {
	...
	read -r cy2 || true
	[ -n "$cy2" ] && CLICK_Y2=$cy2
	printf '%s' "LIGHT_LEVEL [$LIGHT_LEVEL] (POST /light, 1–10): "
	read -r ll || true
	[ -n "$ll" ] && LIGHT_LEVEL=$ll
	...
	lunafast_is_light_level_1_10 "$LIGHT_LEVEL" || {
		printf '%s\n' "LIGHT_LEVEL должен быть целым от 1 до 10."
		read -r _
		return
	}
