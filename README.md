...
В меню: при каждом показе главного экрана выводится **`adb devices -l`**. **1** — поиск в сети (после поиска подменю: прошивка / повтор поиска / главное меню), **2** — установка APK (**выбор устройств по номерам**, опционально **IP:PORT**), **3** — настройки.
