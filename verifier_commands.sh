...
generate_consent() {
  # oid / callback_url / redirect_url — заглушки для валидации тела запроса
  local oid="${CONSENT_OID_STUB:-1000723725}"
  local redirect_url="${CONSENT_REDIRECT_URL:-https://visionlabs.ru}"
  local callback_url="${CONSENT_CALLBACK_URL:-https://visionlabs.ru}"
  ...
  curl -sS --location \
    ...
    --header "Content-Type: application/json" \
    --header "X-API-KEY: ${VERIFIER_X_API_KEY}" \
    --data "{\"oid\": \"${oid}\", \"redirect_url\": \"${redirect_url}\", \"callback_url\": \"${callback_url}\"}"
